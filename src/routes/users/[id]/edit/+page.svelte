<script>
    ...
    async function getUser() {
        const { data } = await axios.get(`http://localhost:8000/api/users/${$page.params.id}`, { withCredentials: true });
    ...
    async function submit() {
        ...
        await axios.put(`http://localhost:8000/api/users/${$page.params.id}`, user, { withCredentials: true });
